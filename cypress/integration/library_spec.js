describe('check kirkendall', () => {
    it('checks for new releases', () => {
        ...
        cy.get('.c-results-utility-result-count').contains('of 52');
    })
    ...
    it('checks new videos', () => {
        ...
        cy.get('.new-releases__link').first().contains('The warrant');
    })
})
...
describe('check bridges audio', () => {
    it('checks for new audiobooks', () => {
        ...
        cy.get('body').then((body) => {
            if (!body.find('.Results-noResultsHeading').length > 0) {
                cy.get('.search-text').contains('of 42 results');
            }
        });
    })
    it('checks for new ebooks', () => {
        ...
        cy.get('body').then((body) => {
            if (!body.find('.Results-noResultsHeading').length > 0) {
                cy.get('.search-text').contains('of 89 results');
            }
        });
    })
