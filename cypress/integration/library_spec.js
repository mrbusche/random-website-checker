describe('check kirkendall', () => {
    ...
    it('checks new videos', () => {
        ...
        cy.get('.new-releases__link a').first().should('have.text', 'The French Dispatch');
    })
})
...
describe('check bridges', () => {
    it('checks for new audiobooks', () => {
        ...
        cy.get('body').then((body) => {
            if (!body.find('.Results-noResultsHeading').length > 0) {
                cy.get('.search-text.h2').first().should('contain.text', '253 results');
            }
        });
    })
    it('checks for new ebooks', () => {
        ...
        cy.get('body').then((body) => {
            if (!body.find('.Results-noResultsHeading').length > 0) {
                cy.get('.search-text.h2').first().should('contain.text', '146 results');
            }
        });
    })
