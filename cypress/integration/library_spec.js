describe('check kirkendall', () => {
    ...
    it('checks new books', () => {
        ...
        cy.get('.new-releases__link a').first().should('have.text', 'The secret lives of customers : a detective story about solving the mystery of customer behavior');
    })

    it('checks new videos', () => {
        cy.visit('https://kplcatalog.ankenyiowa.gov/polaris/Search/newreleases.aspx?ListingTypeID=27');
        cy.get('.new-releases__link a').first().should('have.text', 'The Brokenwood mysteries. Series 7');
    })
})
...
describe('check bridges', () => {
    it('checks for new audiobooks', () => {
        ...
        cy.get('body').then((body) => {
            if (!body.find('.Results-noResultsHeading').length > 0) {
                cy.get('.search-text.h2').first().should('contain.text', '256 results');
            }
        });
    })
    it('checks for new ebooks', () => {
        ...
        cy.get('body').then((body) => {
            if (!body.find('.Results-noResultsHeading').length > 0) {
                cy.get('.search-text.h2').first().should('contain.text', '126 results');
            }
        });
    })
