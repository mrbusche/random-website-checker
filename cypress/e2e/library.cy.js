describe('check kirkendall', () => {
    ...
    it('checks new videos', () => {
        ...
        cy.get('.new-releases__link a').first().should('have.text', "Modern family. The eleventh and final season");
    })
})
    ...
    it('checks for new audiobooks', () => {
        ...
        cy.get('body').then((body) => {
            if (!body.find('.Results-noResultsHeading').length > 0) {
                cy.get('.search-text.h1').first().should('contain.text', '105 results');
            }
        });
    })
