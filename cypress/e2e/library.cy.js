describe('check kirkendall', () => {
    it('checks for new releases', () => {
        ...
        cy.get('.c-results-utility-result-count').eq(1).should(($div) => {
            ...
            expect(text).to.eq('1 - 10 of 37');
        });
    })

    it('checks new books', () => {
        cy.visit('https://kplcatalog.ankenyiowa.gov/polaris/Search/newreleases.aspx?ListingTypeID=26');
        cy.get('.new-releases__link a').first().should('have.text', 'Missing White woman');
    })
...
describe('check bridges', () => {
    it('checks for new audiobooks', () => {
        ...
        cy.get('body').then((body) => {
            if (!body.find('.Results-noResultsHeading').length > 0) {
                cy.get('.search-text.h2').first().should('contain.text', '101 results');
            }
        });
    })
    ...
    it('checks for new ebooks', () => {
        ...
        cy.get('body').then((body) => {
            if (!body.find('.Results-noResultsHeading').length > 0) {
                cy.get('.search-text.h2').first().should('contain.text', '124 results');
            }
        });
    })
