describe('check kirkendall', () => {
    ...
    it('checks new books', () => {
        ...
        cy.get('.new-releases__link a').first().should('have.text', "Bluey and Bingo's fancy restaurant cookbook.");
    })

    it('checks new videos', () => {
        cy.visit('https://kplcatalog.ankenyiowa.gov/polaris/Search/newreleases.aspx?ListingTypeID=27');
        cy.get('.new-releases__link a').first().should('have.text', "Babygirl");
    })
})
    ...
    it('checks for new audiobooks', () => {
        ...
        cy.get('body').then((body) => {
            if (!body.find('.Results-noResultsHeading').length > 0) {
                cy.get('.search-text.h2').first().should('contain.text', '99 results');
            }
        });
    })
