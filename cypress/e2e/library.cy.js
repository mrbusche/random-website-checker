describe('check kirkendall', () => {
    ...
    it('checks new books', () => {
        ...
        cy.get('.new-releases__link a').first().should('have.text', 'Know what matters : lessons from a lifetime of transformations');
    })

    it('checks new videos', () => {
        cy.visit('https://kplcatalog.ankenyiowa.gov/polaris/Search/newreleases.aspx?ListingTypeID=27');
        cy.get('.new-releases__link a').first().should('have.text', 'Gran turismo');
    })
})
