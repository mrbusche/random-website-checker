...
test.describe('check kirkendall', () => {
  ...
  test('checks new books', async ({ page }) => {
    await page.goto('https://kplcatalog.ankenyiowa.gov/polaris/Search/newreleases.aspx?ListingTypeID=26');
    await expect(page.locator('.new-releases__link a').first()).toHaveText('The sun thief');
  });
  ...
  test('checks new videos', async ({ page }) => {
    ...
    await expect(page.locator('.new-releases__link a').first()).toHaveText('Solo mio');
  });
});
...
test.describe('check bridges', () => {
  test('checks for new audiobooks', async ({ page }) => {
    ...
    if (!hasNoResults) {
      await expect(page.locator('.search-text.h1').first()).toContainText('4 results');
    }
  });
  ...
  test('checks for new ebooks', async ({ page }) => {
    ...
    const hasNoResults = (await page.locator('.Results-noResultsHeading').count()) > 0;
    if (!hasNoResults) {
      await expect(page.locator('.search-text.h1').first()).toContainText('8 results');
    }
  });
});
