...
test.describe('check kirkendall', () => {
  test('checks for new releases', async ({ page }) => {
    ...
    expect(text.replace(/\s\s+/g, ' ').trim()).toBe('1 - 10 of 43');
  });
  ...
  test('checks new videos', async ({ page }) => {
    ...
    await expect(page.locator('.new-releases__link a').first()).toHaveText('Kill Bill : the whole bloody affair');
  });
});
...
test.describe('check bridges', () => {
  test('checks for new audiobooks', async ({ page }) => {
    ...
    if (!hasNoResults) {
      await expect(page.locator('.search-text.h1').first()).toContainText('21 results');
    }
  });
